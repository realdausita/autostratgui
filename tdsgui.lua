...
-- // shared for addons
shared.AFK_Table = AFK

-- // ui
loadstring(game:HttpGet("https://raw.githubusercontent.com/KiwiStrats/afk-defense/main/Sources/GuiSource.lua"))()
local Console = shared.KiwiStratsGUI.Console

shared.KiwiStratsGUI.Status(tostring(game_state))
...
local function log(text, color)
    local console_scrolling = shared.KiwiStratsGUI and shared.KiwiStratsGUI.Console
...
local function handle_post_match()
    ...
    local post_data = {
        username = "Kiwi Strats - AFK Defense",
        embeds = {{
            title = (match.Status == "WIN" and "🏆 VICTORY" or "💀 DEFEAT"),
            ...
            footer = { text = "Logged for " .. local_player.Name .. " • Kiwi Strats" },
...
local function log_match_start()
    ...
    local start_payload = {
        ...
        embeds = {{
            ...
            description = "Kiwi Strats has successfully loaded into a new game session and is beginning execution.",
            color = 3447003,
            fields = {
                {
                    name = "🪙 Starting Coins",
            ...
            local ok, res = pcall(function()
                ...
                if ab_data then
                    ...
                    if type(clone_idx) == "number" then
                        data.towerToClone = AFK.placed_towers[clone_idx]
                    end

                    if type(target_idx) == "number" then
                        data.towerTarget = AFK.placed_towers[target_idx]
                    end
                end
    ...
    local res
        repeat
            local ok, result = pcall(function()
                local mode = AFK.matchmaking_map[difficulty]
...
function AFK:Addons()
    local url = "https://api.kiwistrats.dev/api/v1/luascripts/public/afk-defense-addons/download"
    ...
    while not (AFK.MultiMode and AFK.Multiplayer) do
        task.wait(0.1)
    end

    local original_equip = AFK.Equip
    AFK.Equip = function(...)
        if game_state == "GAME" then
...
-- ingame
function AFK:TeleportToLobby()
    send_to_lobby()
end

function AFK:VoteSkip(start_wave, end_wave)
    task.spawn(function()
...
function AFK:GameInfo(name, list)
    list = list or {}
...
function AFK:UnlockTimeScale()
    unlock_speed_tickets()
end

function AFK:TimeScale(val)
    set_game_timescale(val)
end

function AFK:StartGame()
    lobby_ready_up()
end
...
function AFK:GetWave()
    return get_current_wave()
end

function AFK:RestartGame()
    trigger_restart()
end

function AFK:Place(t_name, px, py, pz, ...)
    local args = {...}
    local stack = false
...
function AFK:SetTarget(idx, target_type, req_wave)
    if req_wave then
...
function AFK:Sell(idx, req_wave)
    if req_wave then
        repeat task.wait(0.5) until get_current_wave() >= req_wave
    end
    local t = self.placed_towers[idx]
...
function AFK:Ability(idx, name, data, loop)
    local t = self.placed_towers[idx]
...
function AFK:AutoChain(...)
    ...
    task.spawn(function()
        ...
        while running do
            ...
            local tower = AFK.placed_towers[idx]
...
function AFK:SetOption(idx, name, val, req_wave)
    local t = self.placed_towers[idx]
    if t then
